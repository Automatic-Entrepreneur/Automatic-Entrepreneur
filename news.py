...
def get_news(company: str) -> str:
    ...
    all_articles.extend(
        filter(lambda x: company.lower() not in x["title"], news_results)
    )
    if pipeline:
        sentiments = pipeline("sentiment-analysis")([i["title"] for i in all_articles])
    else:
        sentiments = []
    ...
    if len(sentiments) == 0:
        ...
        content = "".join(
            [
                ...
                for i in all_articles[:5]
            ]
        )
        content += SENTIMENT.format(
            pos=str(
                int(
                    (
                        100
                        * sum([i["label"] == "POSITIVE" for i in sentiments])
                        / len(sentiments)
                    )
                )
            )
